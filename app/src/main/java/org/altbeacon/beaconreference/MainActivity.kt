...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var corre: TextView
    lateinit var textid: TextView
    lateinit var monitoringButton: Button
    lateinit var rangingButton: Button
    lateinit var beaconReferenceApplication: BeaconReferenceApplication
    var alertDialog: AlertDialog? = null
    val ids: Array<String> = arrayOf("1569","1565","1566","1571","1567","1570","7960","7984")
    var myId=ids.random()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        textid = findViewById<TextView>(R.id.myid)
        beaconCountTextView = findViewById<TextView>(R.id.beaconCount)
        corre=findViewById<TextView>(R.id.Correctness)
    ...
    val rangingObserver = Observer<Collection<Beacon>> { beacons ->
        ...
        if (BeaconManager.getInstanceForApplication(this).rangedRegions.size > 0) {
            var ndb=beacons.filter {it -> (it.id3).toString()==myId}
            if(ndb.size>0) {
                if ((ndb[0].distance).toFloat() < 0.5) {
                    corre.text = "Correct"
                    corre.setTextColor(Color.GREEN)
                } else {
                    corre.text = "Incorrect"
                    corre.setTextColor(Color.RED)
                }
            }
            else {
                corre.text = "Incorrect"
            ...
            beaconListView.adapter = ArrayAdapter(this, android.R.layout.simple_list_item_1,
                beacons
                    .filter {it -> (it.id3).toString()==myId}
